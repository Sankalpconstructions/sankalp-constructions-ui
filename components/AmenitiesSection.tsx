...
export default function AmenitiesSection({ projectId }: { projectId?: string }) {
  ...
  return (
    <section id="amenities" className="py-24 bg-white text-gray-900 border-t border-gray-100">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {displayAmenities.map((item, idx) => (
            <motion.div
              ...
              className="glass p-4 rounded-lg flex flex-col items-center justify-center text-center group border border-gray-100 shadow-md hover:shadow-xl transition-all cursor-pointer h-32"
            >
              ...
              <h3 className="text-xs font-bold uppercase text-gray-700 group-hover:text-[#711113] transition-colors leading-tight">
                {item.title}
              </h3>
            </motion.div>
          ))}
